...
<br>
<table cellspacing="5" style="font-size:10px;border-radius:5px;background:#EEEEEE;">
...
{
	if(!$operator->InExternalGroup || $operator->IsBot)
		continue;

    echo "<tr><td><table style=\"width:100%;font-size:10px;font-family:verdana,arial;border-radius:3px;background:#fafafa;\"><td><tr>";
	echo "<tr>";
	echo "<td valign=\"top\">";
	...
	else
	{
		if($operator->Status == USER_STATUS_BUSY)
			...
			echo "<em>Away</em><br>";
		echo "<a href=\"javascript:void(window.open('". LIVEZILLA_PATH . FILE_CHAT . "?intid=".$API->Base64UrlEncode($operator->UserId)."','','width=600,height=600,left=0,top=0,resizable=yes,menubar=no,location=no,status=yes,scrollbars=yes'))\">Leave message</a>";
	}
	echo "<br><br></td>";
	echo "</tr></table></td></tr>";
}
